      ...
      <div v-if="getUsers">
        ...
        >
          <template v-slot:item.actions="{ item }">
            <div class="tw-flex">
              <v-icon class="me-2" size="small" @click.stop="editStatus = true">mdi-pencil</v-icon>
              <v-icon size="small" @click.stop="openDel=true; currentUser = item">mdi-delete</v-icon>
            </div>
          ...
          <template #item="{ item,columns }" v-if="$vuetify.display.name === 'sm'">
            <div class="tw-border tw-my-2 tw-rounded-md tw-border-[gray] tw-p-1 tw-w-[248px]">
              ...
              <v-divider></v-divider>
              <div>
                <div v-for="role in item.roles" class="tw-text-[12px] tw-mx-[3px]">{{role.display_name}}</div>
              ...
              <div>
                <v-btn size="small" color="red" variant="text" @click.stop="openDel=true; currentUser = item">Удалить</v-btn>
  ...
  <v-dialog width="300" v-model="openDel">
    <v-card elevation="0" class="tw-p-2">
      <div class="tw-flex tw-justify-between">
        <div v-if="currentUser">Вы точно хотите удалить  ?</div>
        <div>
          <v-icon @click="openDel = false;currentUser=null">mdi-close</v-icon>
        </div>
      </div>
      <div>
...
export default {
  ...
  methods: {
    ...
    deleteItem(item) {
      this.actUserDelete(this.currentUser.id)
      this.openDel = false
      this.currentUser = null
    },
    ...mapMutations('users', ['setRoles']),
    ...mapActions('users', ['actReqwestUsers', 'actReqwestUserRoles', 'actUserCreate', 'actUserCreateLink','actUserDelete']),
  },
  computed: {
